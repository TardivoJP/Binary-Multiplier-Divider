...
from logic import binaryMultiply, convertDecimalToBinary, convertBinaryArrayToDecimal, generateArr

class MultiplicationInputView(QWidget):
    def __init__(self, show_main_menu_callback):
        ...
        self.manual_input_layout_label = QLabel("<h2>Multiplicacao Sem Sinal</h2>")
        ...
        self.button_script_validate_input = (QPushButton("Multiplicar"))
        self.button_script_validate_input.clicked.connect(self.validate_and_multiply)
        ...
        self.variables_layout_multiplicand_label = QLabel("<h3>Multiplicando</h3>")
        self.variables_layout_multiplicand_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.variables_layout.addWidget(self.variables_layout_multiplicand_label)
        
        self.variables_layout_multiplicand_input = QLineEdit()
        self.variables_layout_multiplicand_input.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.variables_layout_multiplicand_input.setPlaceholderText("Insira o multiplicando")
        self.variables_layout.addWidget(self.variables_layout_multiplicand_input)
        
        self.variables_layout_multiplier_label = QLabel("<h3>Multiplicador</h3>")
        self.variables_layout_multiplier_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.variables_layout.addWidget(self.variables_layout_multiplier_label)
        
        self.variables_layout_multiplier_input = QLineEdit()
        self.variables_layout_multiplier_input.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.variables_layout_multiplier_input.setPlaceholderText("Insira o multiplicador")
        self.variables_layout.addWidget(self.variables_layout_multiplier_input)
        ...
        self.output_layout_label = QLabel("<h2>Passos realizados na multiplicacao binaria sem sinal</h2>")
    ...
    def validate_and_multiply(self):
        multiplicand = self.variables_layout_multiplicand_input.text()
        multiplier = self.variables_layout_multiplier_input.text()
        
        if not self.validate_value(multiplicand, "multiplicando"):
            return
        else:
            multiplicand = int(self.variables_layout_multiplicand_input.text())
            
        if not self.validate_value(multiplier, "multiplicador"):
            return
        else:
            multiplier = int(self.variables_layout_multiplier_input.text())
            
        binary_multiplicand = convertDecimalToBinary(multiplicand)
        binary_multiplier = convertDecimalToBinary(multiplier)
        multiplicand_binary_array = generateArr(binary_multiplicand)
        multiplier_binary_array = generateArr(binary_multiplier)
        
        result, log = binaryMultiply(multiplicand_binary_array, multiplier_binary_array)
        decimalResult = convertBinaryArrayToDecimal(result)
        
        log.insert(0, f"{multiplicand} X {multiplier} = {decimalResult}")
        log.insert(0, "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    ...
    def reset(self):
        self.variables_layout_multiplicand_input.setText("")
        self.variables_layout_multiplier_input.setText("")
        self.output_contents_layout_label.setText("")
